#!/bin/bash
 ...
 RELEASE_ID=$(curl -X POST "https://api.github.com/repos/kgrid/kgrid-library/releases?access_token=$GITHUB_TOKEN" \
                         -H 'Content-Type:application/json' -d "{\"tag_name\":\"$TAG_NAME\",  \"name\":\"KGrid Library $TAG_NAME\", \"prerelease\":$PRERELEASE}" | jq -r ".id")
