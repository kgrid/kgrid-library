<template>
  <olpane layerid=0>
	<div slot="oltitle"><h3>Library Users</h3></div>
	  <div slot="ol-form">
	...
	created:function(){
			var self =this;
			this.$eventBus.$on("userselected", function(user){
      ...
      this.$store.dispatch('fetchuserlist').then(function(){
          self.umodel.userList = self.$store.getters.getuserlist;
      })
      this.$store.dispatch('getcurrentuser')
			this.$eventBus.$on("userAdded", function(user){
				var obj = {};
        ...
        $('.userentry').css("opacity",1);
			...
			this.$eventBus.$on("userUpdated", function(user){
        ...
        $('.userentry').css("opacity",1);
			...
			this.$eventBus.$on('userdeleted',function(userid){
			      console.log(userid)
			    		...
			    		$.ajax({
		    					...
		    					success : function(response) {
		    						if (response != 'empty') {
		    				            ...
		    				            self.$eventBus.$emit('userAdded', self.userModel.user);  // eslint-disable-line
		    				            });
		    						}
		    					},
		    			...
		    			beforeSend : function(xhrObj) {
		    					...
		    					success : function(
		    						...
		    						if (response != 'empty') {
		    				            ...
		    				            $('div.success').fadeIn(300).delay(500).fadeOut(400, function(){  // eslint-disable-line
		    				           	$.extend(true, self.userModel.user, response);
		    				            self.$eventBus.$emit('userUpdated', self.userModel.user);  // eslint-disable-line
		    				            });
		    						}
		    					},
