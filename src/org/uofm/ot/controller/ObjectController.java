		...
		if(validateUser(loggedInUser)) {
			try {
				...
				if(fedoraObject.getMetadata() != null && fedoraObject.getMetadata().getCitations() != null) {
					editFedoraObjectService.editCitations(fedoraObject.getURI(),fedoraObject.getMetadata().getCitations());
				...
				/*
				 * TODO: Remove this code once UI for Edit Citation is fixed.
				 * 
				 * This is for testing edit citation through back end. 
				 * 
				 * 
				FedoraObject newFobj= (FedoraObject ) model.get("fedoraObject");
				if(newFobj.getMetadata() != null && newFobj.getMetadata().getCitations() != null) {
					for (Citation citation : newFobj.getMetadata().getCitations()) {
						String citationTitle = citation.getCitation_title() + "*** Edited";
						String citationAt = citation.getCitation_at() + "*** Edited";
						citation.setCitation_at(citationAt);
						citation.setCitation_title(citationTitle);
					}
					editFedoraObjectService.editCitations(newFobj.getURI(),newFobj.getMetadata().getCitations());
				}*/
				model.addAttribute("ActiveTab", "METADATA");
